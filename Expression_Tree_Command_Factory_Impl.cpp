...
Expression_Tree_Command Concrete_Expression_Tree_Command_Factory_Impl::make_null_command(
    const std::string&)
{
    return Expression_Tree_Command(new Null_Command(tree_context));
}
    ...
    const std::string& input)
{
    ...
    if (space_pos == std::string::npos) {
        parameters = input.substr(space_pos + 1);
        command_keyword = input.substr(0, space_pos);
    } else {
        parameters = input.substr(space_pos + 1);
        command_keyword = input.substr(0, space_pos + 1);
    }
    auto iter = command_map.find(command_keyword);
    if (iter == command_map.end()) {
        std::cout << "ERROR: unrecognizable command "
                  << "\"" << command_keyword << "\"" << std::endl;
        return Concrete_Expression_Tree_Command_Factory_Impl::make_null_command(parameters);
    } else {
