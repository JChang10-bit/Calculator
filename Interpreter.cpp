...
/**
 * @class Modulus
 * @param Modulus node of the parse tree
 */
    ...
    std::list<Symbol*>& list)
{
    ...
    if (is_number(input[i])) {
        ...
        if ((i == input.length() - 1 || i == 0) && is_operator(input[i])) {
        } else {
        ...
        // insert the op according to left-to-right relationships
        if (i == input.length() - 1 && is_operator(input[i])) {
        } else {
            // insert the op according to left-to-right relationships
            precedence_insert(op, list);
        }
